...
- **Runtime controls (between turns only; `errBusy` mid-turn):** `Harness.Compact(ctx, instructions)` forces context compression via the context store (works with any brain); `Harness.SetModel(def)` switches the main model using the harness LLM cache and emits `ModelChangedEvent` (`CurrentModel()` reads it back); `Harness.SetThinking(bool)` toggles reasoning and emits `ThinkingChangedEvent`. SetModel/SetThinking require the default brain (custom `WithAgentBuilder` brains get a clean error unless they implement `SetLLM`/`SetThinking`).
